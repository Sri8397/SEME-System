...
router.post('/addvehicle', [
    ...
    try {
        ...
        let vehicle = await Vehicle.findOne({ vehicle_number: vehicle_number, user: req.user.id });
        if (vehicle) {
            return res.status(400).json({ success, msg: "Sorry this vehicle already exits" });
        }
        vehicle = {
            name, address, state, city, vehicle_number, pincode, user: req.user.id
        };
